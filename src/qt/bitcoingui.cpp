...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Volkshash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and volkshash: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Volkshash Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Volkshash addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Volkshash addresses"));
    ...
    openAction->setStatusTip(tr("Open a volkshash: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Volkshash command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Volkshash network", "", count));
    } else {
...
{
    QString strTitle = tr("Volkshash Core"); // default title
    ...
    // Append title to "Volkshash Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
